...
const updateInterviewMode = async (req, res) => {
    try {
      ...
      if (mode === "ONLINE" && (!google_meet_link || typeof google_meet_link !== "string")) {
        return res.status(400).json({
          success: false,
...
// Update date time và check người phỏng vấn có bị trùng time..
  const updateInterviewDateTime = async (req, res) => {
    try {
      ...
      const interviewerIds = [...new Set(interview.stages.flatMap((stage) => stage.interviewerIds))];
      ...
      if (conflictingInterviews.length > 0) {
        ...
        return res.status(409).json({
          success: false,
          message: "Some interviewers are busy at the selected date and time. Please choose different interviewers or change the date/time.",
