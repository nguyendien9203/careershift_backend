...
const assignInterviewers = async (req, res) => {
    try {
        ...
        if (validInterviewers.length !== interviewerIds.length) {
            return res.status(400).json({ success: false, message: 'One or more interviewer IDs are invalid' });
        }
        ...
        if (conflictingInterviews.length > 0) {
            return res.status(409).json({
                success: false,
                message: 'One or more interviewers are already assigned to another interview at the same time',
        ...
        if (newIds.length === 0) {
            return res.status(200).json({ success: true, message: 'All interviewers already assigned', data: interview });
        }

        stage.interviewerIds.push(...newIds);
        await interview.save();

        res.status(200).json({ success: true, message: 'Interviewers added successfully', data: interview });
    } catch (error) {
